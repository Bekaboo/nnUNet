...
setup_path() {
    ...
    export nnUNet_raw_data_base="${LOCAL_DATA}/raw"
    ...
    export RESULTS_FOLDER="${LOCAL_DATA}/results"
    echo "  -> export RESULTS_FOLDER=${RESULTS_FOLDER}"

    ensure_dir "${nnUNet_raw_data_base}" || return 1
    ensure_dir "${nnUNet_preprocessed}" || return 1
    ensure_dir "${RESULTS_FOLDER}" || return 1
    return 0
}
...
convert_dataset() {
    ...
    local imagesTr="${nnUNet_raw_data_base}/Task001/imagesTr"
    local labelsTr="${nnUNet_raw_data_base}/Task001/labelsTr"
    local dataset_json="${nnUNet_raw_data_base}/Task001/dataset.json"
...
main() {
    ...
    LOCAL_DATA="${2:-${PROJECT_HOME}/local_data/nn_unetv1}"
